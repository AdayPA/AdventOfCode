...
void turn_on(vector<vector<bool>> &grid_, data_ &data)
{
...
void turn_off(vector<vector<bool>> &grid_, data_ &data)
{
    for (int i = data.init_x; i <= data.end_x; ++i)
    {
        for (int j = data.init_y; j <= data.end_y; ++j)
...
void toggle(vector<vector<bool>> &grid_, data_ &data)
{
    for (int i = data.init_x; i <= data.end_x; ++i)
    {
        for (int j = data.init_y; j <= data.end_y; ++j)
...
int countLightsOn(vector<vector<bool>> &grid_)
{
    int count = 0;
...
int main()
{
    vector<vector<bool>> grid_(GRID_SIZE, vector<bool>(GRID_SIZE, false));
    vector<data_> data;
    obtain_data(data);
